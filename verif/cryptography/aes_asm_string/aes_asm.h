...
void AES_ECB_encrypt(struct AES_ctx* ctx, uint32_t* buf);
void AES_ECB_decrypt(struct AES_ctx* ctx, uint32_t* buf);

#define MAX_COLUMNS 16

int stringToHexMatrix(  char *input, uint8_t* output) ;
uint8_t** splitHexToBytes(  char* hexString, int* numRows);
...
{
    //aes_128_enc_key_schedule(RoundKey, Key);
    uint64_t a2,a3,t0;
    uint64_t offset;
    uint8_t i;

  // Load the key into a2 and a3
    asm volatile (
        "ld %0, 0(%2)\n"
        "ld %1, 8(%2)\n"
        : "=r" (a2), "=r" (a3)
...
void Cipher(uint32_t* state,   uint32_t* RoundKey)
{
...
void AES_ECB_encrypt(struct AES_ctx* ctx, uint32_t* buf)
{
  ...
  Cipher((uint32_t*)buf, ctx->RoundKey);
}

void InvCipher(uint32_t* state, const uint32_t* RoundKey)
{
    uint64_t a5,a6,t0,t1,t2,t3,a7,t5;
...
void AES_ECB_decrypt(struct AES_ctx* ctx, uint32_t* buf)
{
  ...
  InvCipher((uint32_t*)buf, ctx->RoundKey);
}
    ...
    for (size_t i = 0; i < numChunks; i++) {
        ...
        for (size_t j = 0; j < 16; j++) {
            ...
            if (index < length) {
                ...
                result[i][j] = (highNibble << 4) | lowNibble;
            } else {
