...
python3 cva6.py --target $DV_TARGET --iss=$DV_SIMULATORS --iss_yaml=cva6.yaml \
    --c_tests ../tests/custom/cryptography/aes_single_block/aes_asm_single_encryption.c \
...
python3 cva6.py --target $DV_TARGET --iss=$DV_SIMULATORS --iss_yaml=cva6.yaml \
    --c_tests ../tests/custom/cryptography/aes_single_block/aes_asm_single_decryption.c \
    --isa_extension zbkb_zbkc_zbkx_zknd_zkne_zknh_zksed_zksh \
    --linker=../tests/custom/common/test.ld \
    --gcc_opts="-static -mcmodel=medany -fvisibility=hidden -nostdlib \
    -nostartfiles -g ../tests/custom/common/syscalls.c \
...
python3 cva6.py --target $DV_TARGET --iss=$DV_SIMULATORS --iss_yaml=cva6.yaml \
    --c_tests ../tests/custom/cryptography/aes_single_block/aes_c_single_encryption.c \
    --isa_extension zbkb_zbkc_zbkx_zknd_zkne_zknh_zksed_zksh \
    --linker=../tests/custom/common/test.ld \
    --gcc_opts="-static -mcmodel=medany -fvisibility=hidden -nostdlib \
    -nostartfiles -g ../tests/custom/common/syscalls.c \
...
python3 cva6.py --target $DV_TARGET --iss=$DV_SIMULATORS --iss_yaml=cva6.yaml \
    --c_tests ../tests/custom/cryptography/aes_single_block/aes_c_single_decryption.c \
    --isa_extension zbkb_zbkc_zbkx_zknd_zkne_zknh_zksed_zksh \
    --linker=../tests/custom/common/test.ld \
    --gcc_opts="-static -mcmodel=medany -fvisibility=hidden -nostdlib \
    -nostartfiles -g ../tests/custom/common/syscalls.c \
