...
int stringToHexMatrix(const char *input, uint8_t output[][MAX_COLUMNS + 1]) ;
uint8_t** splitHexToBytes(const char* hexString, int* numRows);
...
void Cipher(state_t* state, const uint8_t* RoundKey)
{
  ...
  for (size_t i; i<4; i++){
    ...
    {
      printf("%02X ", state[i][j]); // Print each element in 2-digit hex format
    }
    ...
    for (i = 0; i < numRows; i++) {
        for (j = 0; j < MAX_COLUMNS; j++) {
            ...
            if (index < length) {
                ...
                output[i][j] = missingCharLastRow; 
            }
        }
        output[i][MAX_COLUMNS] = '\0';
    }
...
uint8_t** splitHexToBytes(const char* hexString, int* numRows) {
    ...
    *numRows = numChunks;
    uint8_t** result; // Allocate memory for the array of pointers
    if (result == NULL) {
        *numRows = 0;
    ...
    for (size_t i = 0; i < numChunks; i++) {
        result[i]; // Allocate memory for each row
