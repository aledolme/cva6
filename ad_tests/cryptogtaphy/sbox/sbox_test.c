...
void sbox(uint16_t* key_sbox, uint16_t* pt, uint16_t* ct)
{
...
int main() {
    ...
    //Signals for s-box custom instructions
    uint16_t key_sbox = 0x2b7e; // Example key (fixed as simulations)
    uint16_t ct, pt;
    ...
    *trigger = 1 << TRIGGER_CTRL_STOP;
    ...
    //while(1){
        ...
        for (uint32_t i = 0; i < num_traces; i++) {
            ...
            pt = (ciphertext[1] << 8) | ciphertext[0];
