...
int main() {
    ...
    *trigger = 1 << TRIGGER_CTRL_STOP;
    ...
    baud = 12500;       //12500 bps
    init_uart(freq, baud);

    // Test: Print a string over UART
    print_uart("Hello, UART!\n");

    // Allocate seed input buffer
    uint8_t seed_input[AES_BLOCK_SIZE * 2] = {0};
    // Read seed input from UART
    read_seed_input_from_uart(seed_input, AES_BLOCK_SIZE * 2);

    // Confirm the entire seed_input was received
    print_uart("Seed input received:\n");
    for (size_t i = 0; i < AES_BLOCK_SIZE * 2; i++) {
    ...
    for (uint32_t i = 0; i < num_traces; i++) {
        ...
        if (lsb_check) {
            ...
            *trigger = 1 << TRIGGER_CTRL_START;
            ...
            cv_xif_CUS_XOR(&rs1_fixed, &rs2_fixed, &res);
